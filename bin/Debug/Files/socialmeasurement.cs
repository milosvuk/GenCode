...
public class socialmeasurement
 
{
public ArrayList alErrors = new ArrayList();
public string connectionString;
public socialmeasurement()
{
...
public socialmeasurement (System.Int32 _idmeasurement,System.String _codeoflocation,System.DateTime _dateofmeasurement,System.String _codeofindicator,System.Decimal _valueofmeasurement,System.String _description,System.Int32 _weekm,System.Int32 _monthm,System.Int32 _yearm,System.Int32 _idcustomer )
{
...
int numberinsert=0;
// INSERT INTO socialmeasurement(idmeasurement,codeoflocation,dateofmeasurement,codeofindicator,valueofmeasurement,description,weekm,monthm,yearm,idcustomer ) VALUES(idmeasurement,codeoflocation,dateofmeasurement,codeofindicator,valueofmeasurement,description,weekm,monthm,yearm,idcustomer );
...
////==========================================================================
public bool Save(socialmeasurement _socialmeasurement)
{
bool tmp = false;
int res = -1;
_socialmeasurement.DELETED = 1;
...
try
{
DB_socialmeasurement db_socialmeasurement = new DB_socialmeasurement();
if (_socialmeasurement.IDSOCIALMEASUREMENT == 0)
{
res = db_socialmeasurement.Add(_socialmeasurement, conn, tran);
tran.Commit();
tmp = true;
}
else
{
res = db_socialmeasurement.Update(_socialmeasurement, conn, tran);
tran.Commit();
tmp = true;
}
}
...
{
int numberedit=0;
string mupdate = "UPDATE socialmeasurement SET "+"codeoflocation="+(char)39+codeoflocation.ToString()+(char)39+(char)44+"dateofmeasurement="+(char)39+dateofmeasurement.ToString()+(char)39+(char)44+"codeofindicator="+(char)39+codeofindicator.ToString()+(char)39+(char)44+"valueofmeasurement="+valueofmeasurement.ToString()+(char)44+"description="+(char)39+description.ToString()+(char)39+(char)44+"weekm="+weekm.ToString()+(char)44+"monthm="+monthm.ToString()+(char)44+"yearm="+yearm.ToString()+(char)44+"idcustomer="+idcustomer.ToString()+" WHERE idmeasurement="+idmeasurement.ToString();
...
{
int numberdelete=0;
string del = "DELETE FROM socialmeasurement WHERE idmeasurement="+idmeasurement.ToString();
...
string alistsearch = "SELECT * FROM socialmeasurement " +SearchExp;
ptl.DbaseSQLServer sq= new ptl.DbaseSQLServer();
sq.DbaseConnection = Con;
...
{
ArrayList alsearch = new ArrayList();
string alistsearch = "SELECT * FROM socialmeasurement";
ptl.DbaseSQLServer sq= new ptl.DbaseSQLServer();
sq.DbaseConnection = Con;
sq.SqlText = alistsearch;
alsearch = sq.RunViewSQL("|");
...
string dtsearch = "SELECT * FROM socialmeasurement " +SearchExp;
ptl.DbaseSQLServer sq= new ptl.DbaseSQLServer();
sq.DbaseConnection = Con;
sq.SqlText = dtsearch;
dt = sq.RunViewSQL();
...
{
DataTable dt = new DataTable();
string dtsearch = "SELECT * FROM socialmeasurement";
ptl.DbaseSQLServer sq= new ptl.DbaseSQLServer();
sq.DbaseConnection = Con;
sq.SqlText = dtsearch;
dt = sq.RunViewSQL();
